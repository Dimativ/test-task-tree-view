...
export const Navigation = () => {
    ...
    }, [pathname, isMobile]); //working with pathname change
    ...
    const handleMouseDown = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
        ...
        document.addEventListener("mousemove", handleMouseMove); //using to resize the sidebar, but when we leave we stop resizing
        document.addEventListener("mouseup", handleMouseUp);
    };

    const handleMouseMove = (event: MouseEvent) => {
        if (!isResizingRef.current) return; // break
        let newWidth = event.clientX;

        //how much the user can resize and collapse the sidebar
    ...
    const resetWidth = () => {
        if (sidebarRef.current && navbarRef.current) {
            ...
            setIsResetting(true); // because need animation the resetting of the width
            ...
            }, 300); // using therefore transition of 300
        }
    };
